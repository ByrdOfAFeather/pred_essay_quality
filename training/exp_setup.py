...
=======
    ...
    for lr in grids["lr"]:
        for max_length in grids["max_length"]:
            for batch_size in grids["batch_size"]:
                for model_index in grids["model_index"]:
                    #model = BertClassifier(dropout=0.1, weight_grad_index=model_index, loss_weights=[1.0, 1.0, 1.0], num_labels=3)
                    ...
                    train_set = tokenized["train"].shuffle(seed=225530)
                    val_set = tokenized["test"].shuffle(seed=225530)
                    eval_results = train(model, train_set, val_set, weights_list=[1.0, 1.0, 1.0], training_args=training_args, name_format=name_format)
                    if prev_name:
